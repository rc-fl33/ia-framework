...
**Receives:**
- All deliverable files from Phase 4 in `private/output/advisory/{type}/{project}-{date}/`
- Mode (ad-hoc, code-review)
- Metadata from metadata.json (arch/code-review)
...
**Source:** `skills/advisory/phases/04-document.md`

---

## OBJECTIVE

**Goal:** Present the complete advisory engagement results to the user with clear summary, critical item emphasis, and actionable follow-up guidance.
...
**Phase 5 is the handoff.** The user should walk away knowing exactly what was found, where the reports are, what to fix first, and what tools to use next. Everything should be clear and actionable.

**Critical items get special attention.** P0 findings represent immediate risks. They must be presented prominently and not buried in a summary table. If P0 items exist, lead with them.

**Follow-up suggestions convert findings to action.** An architecture review that finds vulnerabilities should suggest `/pentest` for validation. A code review finding compliance gaps should suggest `/compliance`. Infrastructure issues suggest `/harden`. These connections help the user take the next step.
...
Primary Deliverable
FULL-REPORT.md - Consolidated report with all analysis, findings, and recommendations

Start with FULL-REPORT.md for complete context. Individual files available for
isolated sections or detailed reference.

All Deliverables

| File | Description |
|------|-------------|
| FULL-REPORT.md | Consolidated report (recommended starting point) |
| [file1] | [description] |
| [file2] | [description] |
| ... | ... |
...
| Architecture concerns | Use `/sec-review` for full architecture threat modeling |
...
```
[SKILL:advisory] completed {mode} for {project}

Deliverables: private/output/advisory/{type}/{project}-{date}/
```

**AFTER THIS STEP:** Display the grand summary using the workflow-level format. Build dynamically from actual phase outputs — mode, metrics, file lists, and deliverable counts. See `/docs/guides/running-checklist-standard.md` for the exact grand summary format.
...
- All deliverables from Phase 4 remain in `private/output/advisory/{type}/{project}-{date}/`
...
**On user requesting additional analysis:** → Start new workflow for different scope or mode.
...
- Request a different advisory mode (e.g., code-review after advisory guidance)
