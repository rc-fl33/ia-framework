...
**Receives:**
- Analysis output from Phase 2 (FINDINGS.md)
- Scope from Phase 1
...
**Source:** `skills/code-review/phases/02-analyze.md`
...
**Framework mapping adds credibility.** When a recommendation maps to NIST CSF 2.0 or OWASP controls, it carries more weight with stakeholders.
...
**Tool:** Read
**Reference:** Phase 2 output files in `private/output/code-review/{project}-{YYYY-MM-DD}/`
...
**Confidence-aware prioritization** — apply these adjustments after initial priority assignment:

- **HIGH confidence findings:** May warrant elevation beyond severity alone (a HIGH confidence
  Medium severity finding may be more actionable than a LOW confidence Critical)
- **LOW confidence findings:** Cap at P2 unless the pattern is Critical severity AND no
  mitigations were found — in that case, retain P1 but note "verify exploitability before
  treating as P1"
- **LOW confidence findings at P2:** Add note: "verify exploitability before treating as P2"
- Priority is the intersection of severity × confidence — a LOW confidence P0 should include a
  verification step before escalation

**Expected output:** All findings assigned P0-P3 priority with confidence-adjusted notes where applicable
...
|--------------|---------------------|
| Authentication | NIST PR.AA-*, OWASP A07, API3 (Broken Object Property Level Auth) |
| Input validation | CWE-20, OWASP A03, API1 (Broken Object Level Authorization) |
| Encryption | NIST PR.DS-*, PCI-DSS 3.x |
| Access control | NIST PR.AC-*, OWASP A01, API1/API5 (BOLA/BFLA) |
| Logging | NIST DE.CM-*, CIS Control 8, API9 (Improper Inventory Management) |
| Configuration | CIS Controls, NIST PR.IP-* |
| API security | OWASP API Top 10 (API1-API10) — apply when routes, OpenAPI specs, or REST/GraphQL handlers detected |
| Business logic | OWASP API6 (Unrestricted Resource Consumption), API8 (Security Misconfiguration) |
...
**Produces:**
- `RECOMMENDATIONS.md` → written to `private/output/code-review/{project}-{YYYY-MM-DD}/RECOMMENDATIONS.md`
...
Load `skills/code-review/phases/04-document.md` with:
